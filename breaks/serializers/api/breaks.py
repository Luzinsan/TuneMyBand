...
class BreakMeCreateSerializer(InfoModelSerializer):
    ...
    def validate(self, attrs):
        ...
        member = ReplacementMember.objects.filter(
            ...
            member__participant__user=user
        ).first()

        if not member:
            raise ParseError('У вас нет доступа к текущему репетиционному дню.')
        ...
        if break_start + max_duration > break_end:
            raise ParseError(
                'Продолжительность репетиции превышает максимальное установленное значение.'
            )

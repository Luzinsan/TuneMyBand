...
from bands.serializers.nested.groups import GroupShortSerializer
...
class ReplacementCreateSerializer(InfoModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if attrs.get('break_start') and attrs.get('break_end'):
            if attrs.get('break_start') >= attrs.get('break_end'):
                raise ParseError(
                    'Время начала репетиции должно быть меньше времени окончания.'
                )
        ...
        if self.Meta.model.objects.filter(
            ...
            raise ParseError(
                'На этот день уже существует активный репетиционный день.'
            )
        return attrs
    ...
    def validate_group(self, value):
        ...
        if value not in my_groups:
            raise ParseError(
                'У вас нет полномочий создавать репетиционные дни в этой группе.'
            )
        return value
    ...
    def validate_date(self, value):
        ...
        if value < now:
            raise ParseError(
                'Дата репетиционного дня должна быть больше или равна текущей дате.'
            )
        return value
    ...
    def validate_break_start(self, value):
        if value.minute % 15 > 0:
            raise ParseError(
                'Время начала репетиции должно быть кратно 15 минутам.'
            )
        return value
    ...
    def validate_break_end(self, value):
        if value.minute % 15 > 0:
            raise ParseError(
                'Время окончания репетиции должно быть кратно 15 минутам.'
            )
        return value
...
class ReplacementUpdateSerializer(InfoModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if attrs.get('date') and self.Meta.model.objects.filter(
            ...
            raise ParseError(
                'Этот день уже является репетиционным.'
            )
        return attrs
